...
function Log(level) {
  ...
  this.levels = [
    ...
    { type: 'error', color: 'red', fn: this._logFatal },
    { type: 'fatal', color: 'red', fn: this._logError }
  ];
...
/**
 * Custom output method for 'error' level (caught exceptions). Prints
 * the stack trace.
 * @param  {Error} err
 * @private
 */
Log.prototype._logError = function(err) {
  console.error(this.prefix, 'caught exception'.red, '\n');
