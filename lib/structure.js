...
/**
 * Returns the event with the given category and event name
 * @param  {String} category
 * @param  {String} event
 * @return {Object}
 * @public
 */
Structure.prototype.getEvent = function(category, event) {
  if (!this.eventExists(category, event)) {
    throw new Error('The event "' + category + '.' + event + '" does not exist.');
  }
  return this.categories[category].events[event];
};
...
/**
 * Returns a hash containing the field type for each field of
 * each event of each category
 * @return {Object}
 * @public
 */
