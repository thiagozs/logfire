...
LogfireEventsStore.prototype.create = function(event) {
  if (!event.event) throw new LogfireError('Event is not given.');
...
LogfireEventsStore.prototype._addToSet = function(event) {
  var setKey = this.config.prefix + 'set:' + event.event;
...
LogfireEventsStore.prototype._createIndexes = function(event) {
  var structure = this.logfire.structure;
  var fields = structure.getEventFields(event.event);
  var promises = [];
  for (var fieldName in fields) {
    ...
    var indexKey = this.config.prefix + 'indexes:' + event.event + ':' + fieldName;
